...
Connection.prototype.end = function end(options, callback) {
  ...
  this._implyConnect();
  this._preparedStatements.forEach(function (element) {
    // Send deallocate for each valid PreparedStatement element and remove
    // refernce in and to this Connection
    if (element) {
      element._end();
    }
  });
  // Reset PreparedStatements stack
  this._preparedStatements = [];
...
Connection.prototype.destroy = function() {
  ...
  this._preparedStatements.forEach(function(element, index, arr) {
    // remove reference to all valid PreparedStaments, bound to this connection
    if (element) {
...
/**
 * Push a new PreparedStatement to the Connection._preparedStatemnts stack and
 * returns this PreparedStatement.
 * @returns {PreparedStatement} To use it, you must assign the returned Object
 * to a variable in your code
 */
Connection.prototype.getPreparedStatement = function () {
  var prpStmt = new PreparedStatement(this);
  this._preparedStatements.push(prpStmt);
  return prpStmt;
};
